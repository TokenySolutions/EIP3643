...
The T-REX (ERC-42) token is an institutional grade security token standard, developed on Ethereum. <br>
This standard provides a library of interfaces for the management and transfer of security tokens. It allows the contract owner/agent to manage the rules that the transfers have to fulfill to be considered as valid transfers as well as the rules related to an onchain identity to consider its owner as an eligible participant in the token ecosystem. The standard also provides a full set of functions to allow the token agent to manage the token properly.
...
Give standard interfaces for security tokens issued on Ethereum, through which any third party could interact with the security token. <br>
...
The following requirements have been compiled following discussions with parties across financial institutions that are looking to issue securities on a DLT infrastructure such as ethereum.<br>

- MUST be ERC-20 ([#20](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-20.md)) compatible.
...
Transfers of securities can fail for a variety of reasons. This is in direct contrast to utility tokens, of which generally only require the sender to have a sufficient balance. <br>
...
Description of other functions of the ERC-42 can be found in the `interfaces` folder. An implementation of the ERC-42 suite of smart contracts can be found on the [T-REX repository](https://github.com/TokenySolutions/T-REX). 

### Token interface

ERC-42 permissioned tokens are based on a standard ERC-20 structure but with some functions being added in order to ensure compliance in the transactions of the security tokens. The functions `transfer` and `transferFrom` are implemented in a conditional way, allowing them to proceed with a transfer only IF the transaction is valid. The permissioned tokens are allowed to be transferred only to validated counterparties, in order to avoid tokens being held in wallets/ONCHAINIDs of ineligible/unauthorized investors. The ERC-42 standard also supports the recovery of security tokens in case an investor loses his/her wallet private key. A history of recovered tokens is maintained on the blockchain for transparency reasons. ERC-42 tokens are implementing a lot of additional functions to give the owner or his agent the possibility to manage supply, transfer rules, lockups and everything that could be required in the management of a security. <br>
A detailed description of the functions can be found in the [interfaces folder](https://github.com/tokenyICO/eip-42/tree/main/interfaces). <br>
...
### Total on-chain validation

This standard is working completely on-chain. No off-chain validator is needed to perform transactions and token exchanges, thus introducing no external dependencies for the regulation of the exchanges.

### KYC/AML Considerations

Under some jurisdictions, a KYC/AML screening of one’s identity is necessary to hold tokens representing an underlying financial instrument. This standard relies heavily on identity contracts and validation through claims, enforcing the screening of identities before being able to hold tokens.

### Cap-table immutability
Through heavy identity integration, the standard is able to ensure the immutability of the cap-table on the blockchain. Since tokens are linked to identities instead of wallets, it is always possible to trace to whom the token belongs to, even in the case of lost wallets.

## References

- [ONCHAINID documentation](https://docs.onchainid.com/)
- [T-REX White Paper](https://tokeny.com/wp-content/uploads/2019/12/Whitepaper-T-REX-Security-tokens.pdf)
